...
import { Star, ArrowRight, ArrowLeft, RadioTower } from "lucide-react";

export const dynamic = "force-dynamic";

export default async function UsoapPelTrackerPage() {
  const all = await prisma.protocolQuestion.findMany({
    select: { pqNo: true, ce: true, status: true, isPPQ: true, isATC: true },
  });
  ...
  const ceStats = ceIds.map((ce) => {
    ...
    return {
      ...
      atcCount: pqs.filter((p) => p.isATC).length,
  ...
  return (
    <div className="max-w-6xl">
      ...
      >
        <ArrowLeft size={14} /> Flight Safety Department
      </Link>

      <header className="mb-8">
        <div className="text-[10px] uppercase tracking-[0.2em] text-ink-400">
          USOAP CMA Management · Personnel Licensing · Annex 1
        </div>
        <h1 className="text-3xl font-semibold text-ink-50 mt-1 tracking-tight">
          USOAP CMA Tracker
        </h1>
        <p className="text-sm text-ink-300 mt-1">
          Oman Civil Aviation Authority · ICAO USOAP CMA · Protocol Questions
